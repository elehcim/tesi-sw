...
DV=dv_total(logical(tracers_in_SOI(Index)));
T=transfer_time(Index);
scatter(T*11.86/(2*pi)*12,DV,40,'filled')
ylim([0 20])
...
% y_=meshgrid(tr.vx(Index),tr.vy(Index));
color_span=(dv_total-min(dv_total))/(max(dv_total)-min(dv_total))*(2.2-1.1)+1.1;
scatter3(tr.vx,tr.vy,dv_total, 15,color_span,'filled');
% g=gridfit(tr.vx,tr.vy,dv_total,100,100);
% xcoord=linspace(min(tr.vx),max(tr.vx),100);
% ycoord=linspace(min(tr.vy),max(tr.vy),100);
% F=TriScatteredInterp(tr.vx',tr.vy',dv_total')
% zcoord=F(xcoord,ycoord);
% mesh(xcoord,ycoord,zcoord);

zlabel('$\Delta v$','interpreter','latex','fontsize',35)
% for j=1:tr.n_tracers
% plot3([tr.vx(j),tr.vx(j)],...
% 	[tr.vy(j),tr.vy(j)],...
% 	[0,color_span(j)],'k', 'linewidth',3)
% end
% FIXME adjust z axis
box off
view(3)
