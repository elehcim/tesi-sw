...
typedef thrust::device_vector< bool> bool_vec_type;
...
int tracers (double Ki, std::vector<double> obs_times)
{
    ...
    thrust::device_vector< double > X_tracers (5*dim_tracers,0);
    create_integration_vector_t(X_tracers, dim_tracers, x0t, y0t, vx0t, vy0t, e0t, filter_tracers);
    int_vec_type Collisions_t(dim_tracers,0);
    er3bp_system tracers_system(dim_tracers, Collisions_t);
    ...
    print_tracers(tracers_stream, X_tracers, dim_tracers,obs_times[0], filter_tracers);
    for (int i=0; i< n_frames-1; i++)
    {
        integrate_adaptive(make_controlled(abs_tol, rel_tol, stepper_type()), tracers_system,X_tracers, obs_times[i],obs_times[i+1],1e-6);
