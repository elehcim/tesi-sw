...
int create_missing_vector_t(int id, double1d& x_0, double1d& y_0, double1d& vx_0, double1d& vy_0, double1d& e_0, double4d& filter_integration, double Ki )
{
    int c=0;
    double missing_coordinate;
    double1d missing_coordinate_vec(n_tot_t);
    for (int i=0; i<nxt; i++)
        {
            for (int j=0; j<nyt; j++)
            {
                for (int k=0; k<n1t; k++)
                {
                    for(int l=0; l<n2t; l++)
                    {
                        if (flags_t[2]==0) /* missing coordinate is vx */
                        {
                            missing_coordinate=-copysign(1.00, y_0[j])*sqrt(2*Omega(x_0[i],y_0[j],mu)/Ki+2*e_0[l]-vy_0[k]*vy_0[k]);
                        }
                        if (flags_t[3]==0) /* missing coordinate is vy */
                        {
                            missing_coordinate=copysign(1.00, x_0[i])*sqrt(2*Omega(x_0[i],y_0[j],mu)/Ki+2*e_0[l]-vx_0[k]*vx_0[k]);
                        }
                        if (flags_t[4]==0) /* missing coordinate is e */
                        {
                            missing_coordinate=0.5*(vx_0[k]*vx_0[k]+vy_0[l]*vy_0[l])-Omega(x_0[i],y_0[j],mu)/Ki;
                        }
        ...
        if (flags_t[2]==0){vx_0=missing_coordinate_vec;}
        if (flags_t[3]==0){vy_0=missing_coordinate_vec;}
        if (flags_t[4]==0){e_0=missing_coordinate_vec;}
    return c;
}
