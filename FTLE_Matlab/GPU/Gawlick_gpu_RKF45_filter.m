...
%In this approach, only useful points are stored and integrated
		...
		for k=1:nvx
			for l=1:ne
				...
				if ~((j~=2) && (l~=2)) && isreal(v_y)
					% create monodimensional array of useful points
					x(m)=x_0(i);
					y(m)=y_0(j);
					vx(m)=vx_0(k);
					e(m)=e_0(l);
					vy(m)=v_y;
					% useful point implies filter=1
					filter(i,j,k,l)=1;
					m=m+1;
				end
			end
...
fprintf('number of useful points: %i\n',m);
...
[x_f,y_f,vx_f,vy_f]=arrayfun(@RKF45_gpu,t0,T,x_gpu,y_gpu,vx_gpu,vy_gpu,mu);
...
%% Construction of matrix for FTLE computation from monodimensional arrays
...
%% FTLE computation
[ftle, dphi]=Compute_FTLE_gpu( X_T, Y_T, VX_T, E_T, dx, dy, dvx, de, T, filter_ftle);

%% For plotting purpouse the first row and column are deleted
