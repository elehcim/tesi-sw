function [ x_T, y_T, vx_T, e_T, filter, delta_e ] = Integrate_FTLE_Gawlick_ell( x_0, y_0, vx_0, e_0, F, mu, ecc, ni, options)
%Integrate
...
for i=1:nx
	...
	for j=1:ny
		parfor k=1:nvx
			for l=1:ne
				if useful(i,j,k,l)
					vy_0(i,j,k,l)=-sqrt(2*(Omega(x_0(i),y_0(j),mu)/(1+ecc*cos(ni)))+2*e_0(l)-vx_0(k)^2);
					if isreal(vy_0(i,j,k,l))
						filter(i,j,k,l)=1;
						
						ci=[x_0(i), y_0(j), vx_0(k), vy_0(i,j,k,l)];
						[t,Y]=ode45(@f_ell,[ni, F+ni], ci, options, mu, ecc);
						
						if abs(t(end)) < abs(F) % Consider also negative time
							filter(i,j,k,l)=3
						end
						...
						e_T(i,j,k,l)=0.5*(vx_T(i,j,k,l)^2+vy_T(i,j,k,l)^2)-Omega(x_T(i,j,k,l),y_T(i,j,k,l),mu)/(1+ecc*cos(F+ni));
