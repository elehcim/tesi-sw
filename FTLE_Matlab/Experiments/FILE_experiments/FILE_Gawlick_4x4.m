...
for i=1:n
    for j=1:n
        if filter_ftle(i,j)
			...
			dphi(1,2)=0;
            dphi(1,3)=(x_T(i,j+1)-x_T(i,j-1))/dvx;
			dphi(1,4)=0;
            dphi(2,1)=(y_T(i+1,j)-y_T(i-1,j))/dx;
            dphi(2,2)=0;
            dphi(2,3)=(y_T(i+1,j)-y_T(i-1,j))/dvx;
            dphi(2,4)=0;
			dphi(3,1)=(vx_T(i+1,j)-vx_T(i-1,j))/dx;
            dphi(3,2)=0;
			dphi(3,3)=(vx_T(i,j+1)-vx_T(i,j-1))/dvx;
            dphi(3,4)=0;
            dphi(4,1)=(vy_T(i+1,j)-vy_T(i-1,j))/dx;
            dphi(4,2)=0;
            dphi(4,3)=(vy_T(i+1,j)-vy_T(i-1,j))/dvx;
            dphi(4,4)=0;
...
shading flat
t_tot=toc
nome=['var_xvx_', 'ode00', '_n',num2str(n),'_FILE_Gawlick_4x4'];
save(nome)