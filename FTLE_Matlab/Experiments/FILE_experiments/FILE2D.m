...
for i=1:n
	...
	parfor j=1:n
		if E_cin(i,j)>0 && isreal(vy_0(i,j)) % Check for real velocity and positive Kinetic energy
            [t,Y,te,ye,ie]=ode45(@f,[0 T],[x_0(i,j); y_0; vx_0(i,j); vy_0(i,j)],options,mu);
			a(i,j)=length(Y);
            % Saving solutions
			x_T(i,j)=ye(N+1,1);
			vx_T(i,j)=ye(N+1,3);
...
%% FILE Computation
fprintf('adesso calcolo file\n')
tic
dphi=zeros(2,2);
file=zeros(n-2,n-2);
file_norm=zeros(n-2,n-2);
	...
	for j=2:n-1
		if filtro_1(i,j) && ... % Check for interesting point
			...
			if filtro_1(i,j)==2 % Manual setting to visualize bad integrated points 
				...
				file(i-1,j-1)=(1/abs(N))*log(max(sqrt(abs(eig(dphi*dphi')))));
                file_norm(i-1,j-1)=(1/abs(N))*log(norm(dphi));
			end
		end
	end
end
...
t_file=toc;
fprintf('tempo per integrare      \t%.2f s\n',t_integrazione)
fprintf('tempo per calcolare file \t%.2f s\n',t_file)

% ora=fstringf %TODO
% save(['var_' num2str(n) '_' num2str(clock(4)])

nome=['var_xvx_', 'ode00', '_n',num2str(n),'_FILE'];
save(nome)